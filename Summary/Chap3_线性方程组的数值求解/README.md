...
$$||x||_{\infin}=max\{|x_1|+|x_2|+\dots+|x_n|\}$$
...
$$||A||_{\infin}=\max_{1\leq i\leq n}\left\{\sum_{j=1}^n|a_{ij}| \right\}$$

$$||A||_2=\sqrt{\lambda_{\max}(A^TA)}$$其中$\lambda_{\max}(A^TA)$ 表示 $A^TA$ 的最大特征值。

Frobenius范数$$||A||_F=\sqrt{\sum_{i=1}^n\sum_{j=1}^na_{ij}^2}$$
...
$$x_i^{(k+1)}=-\dfrac{1}{a_{ii}}\left(\sum_{j=1}^{i-1}a_{ij}x_j^{(k+1)}+\sum_{j=i+1}^{n}a_{ij}x_j^{(k)}\right)+\dfrac{b_i}{a_{ii}}\tag{2}$$
...
$$x^{(k+1)}=Mx^{(k)}+f,\enspace k=0, 1, 2, \dots \tag{\textrm{I}}$$

由 $(\textrm{I})$ 式产生的向量序列收敛且极限与初值无关**当且仅当** $\rho(M)<1$ 即矩阵谱半径小于 $1$。(迭代法可用的条件)

**谱半径：** $\rho(M)=\max\{|\lambda_1|, |\lambda_2|, \dots, |\lambda_n|\}$，即特征值最大值